...
--   versionUrl   : URL returning JSON {"version":"x.y.z"}
...
--- Check for updates by fetching a version from a remote URL (JSON only: {"version":"x.y.z"}).
--- Prints a coloured warning in the server console if outdated.
---@param versionUrl  string   URL returning JSON {"version":"x.y.z"}
...
TS.CheckUpdate = function(versionUrl, changelogUrl)
    ...
    PerformHttpRequest(versionUrl, function(statusCode, responseText, _)
        ...
        -- JSON only: expect {"version":"x.y.z"} (optional "v" prefix stripped)
        local latestVersion = responseText:match('"version"%s*:%s*"[vV]?([%d%.]+)"')

        if not latestVersion or latestVersion == '' then
            print(('^3[%s] Update check: response is not valid JSON with "version" key.^0'):format(resourceName))
            return
        end
...
if GetCurrentResourceName() == 'ts-lib' then
    CreateThread(function()
        Wait(3000)
        local url = Config and Config.UpdateCheckURL
        if url and url ~= '' then
            TS.CheckUpdate(url, Config.UpdateCheckChangelogURL or '')
        end
    end)
end
